...
# 主Frame外包裹一层白色背景Frame用于居中和填充
        ...
        main_frame.grid_columnconfigure(0, weight=1, minsize=scale_size(self.root, 120))
        main_frame.grid_columnconfigure(1, weight=4, minsize=scale_size(self.root, 600))
        main_frame.grid_columnconfigure(2, weight=1, minsize=scale_size(self.root, 120))
    ...
    def setup_left(self, parent):
        ...
        # 泵喷速度输入区（竖直布局）
        thrust_input_frame = ttk.Frame(thruster_frame)
        thrust_input_frame.pack(fill=tk.X, padx=scale_size(self.root,8), pady=(0,scale_size(self.root,4)))
        ttk.Label(thrust_input_frame, text="左泵喷").grid(row=0, column=0, padx=scale_size(self.root,4), pady=scale_size(self.root,2), sticky="w")
        self.left_thrust_var = tk.StringVar(value="1500")
        left_entry = ttk.Entry(thrust_input_frame, textvariable=self.left_thrust_var, width=8)
        left_entry.grid(row=1, column=0, padx=scale_size(self.root,4), pady=scale_size(self.root,2))
        ttk.Label(thrust_input_frame, text="右泵喷").grid(row=2, column=0, padx=scale_size(self.root,4), pady=scale_size(self.root,2), sticky="w")
        self.right_thrust_var = tk.StringVar(value="1500")
        right_entry = ttk.Entry(thrust_input_frame, textvariable=self.right_thrust_var, width=8)
        right_entry.grid(row=3, column=0, padx=scale_size(self.root,4), pady=scale_size(self.root,2))
        def send_thrust_cmd():
            try:
                left = int(self.left_thrust_var.get())
                right = int(self.right_thrust_var.get())
                left = max(1050, min(2000, left))
                right = max(1050, min(2000, right))
                cmd = f"WF,{left},{right}"
                self.send_command(cmd, "自定义推进器速度", target="motion")
                self.log(f"已发送泵喷速度命令: {cmd}")
            except Exception as e:
                self.log(f"泵喷速度输入错误: {e}")
        send_btn = ttk.Button(thrust_input_frame, text="发送泵喷速度", command=send_thrust_cmd)
        send_btn.grid(row=4, column=0, padx=scale_size(self.root,8), pady=scale_size(self.root,2))
        ...
        # 履带速度模式切换
        self.track_high_speed = False
        def toggle_track_speed():
            self.track_high_speed = not self.track_high_speed
            if self.track_high_speed:
                speed_btn.config(text="切换为低速模式")
                self.setup_track_buttons(high_speed=True)
                self.log("履带已切换为高速模式")
            else:
                speed_btn.config(text="切换为高速模式")
                self.setup_track_buttons(high_speed=False)
                self.log("履带已切换为低速模式")
        speed_btn = ttk.Button(track_frame, text="切换为高速模式", command=toggle_track_speed)
        speed_btn.pack(fill=tk.X, padx=scale_size(self.root,8), pady=(0,scale_size(self.root,4)))

        # 履带控制按钮十字布局
        track_cross = ttk.Frame(track_frame)
        track_cross.pack(pady=scale_size(self.root,4))

        self.track_forward_btn = ttk.Button(track_cross, text="⬆\n前进", width=7)
        self.track_forward_btn.grid(row=0, column=1, padx=scale_size(self.root,4), pady=scale_size(self.root,4))

        self.track_left_btn = ttk.Button(track_cross, text="⬅\n左转", width=7)
        self.track_left_btn.grid(row=1, column=0, padx=scale_size(self.root,4), pady=scale_size(self.root,4))

        self.track_stop_btn = ttk.Button(track_cross, text="⏹\n停止", width=7, command=lambda: self.send_command("TS" if not self.track_high_speed else "US", "履带停止", target="motion"))
        self.track_stop_btn.grid(row=1, column=1, padx=scale_size(self.root,4), pady=scale_size(self.root,4))

        self.track_right_btn = ttk.Button(track_cross, text="➡\n右转", width=7)
        self.track_right_btn.grid(row=1, column=2, padx=scale_size(self.root,4), pady=scale_size(self.root,4))

        self.track_backward_btn = ttk.Button(track_cross, text="⬇\n后退", width=7)
        self.track_backward_btn.grid(row=2, column=1, padx=scale_size(self.root,4), pady=scale_size(self.root,4))

        # 初始化履带按钮命令
        self.setup_track_buttons(high_speed=False)

        # 红色圆形紧急停止按钮
        stop_btn = ttk.Button(parent, text="●\n紧急停止", style="Error.TButton", command=self.emergency_stop, width=7)
        stop_btn.pack(pady=scale_size(self.root,12))

    def setup_track_buttons(self, high_speed=False):
        # 动态绑定履带按钮命令
        if high_speed:
            self.setup_long_press(self.track_forward_btn, "UF", "履带高速前进", "US", "履带停止", target="motion")
            self.setup_long_press(self.track_backward_btn, "UB", "履带高速后退", "US", "履带停止", target="motion")
            self.setup_long_press(self.track_left_btn, "UL", "履带高速左转", "US", "履带停止", target="motion")
            self.setup_long_press(self.track_right_btn, "UR", "履带高速右转", "US", "履带停止", target="motion")
        else:
            ...
            self.setup_long_press(self.track_left_btn, "TL", "履带左转", "TS", "履带停止", target="motion")
            self.setup_long_press(self.track_right_btn, "TR", "履带右转", "TS", "履带停止", target="motion")
    ...
    def setup_center(self, parent):
        ...
        self.video_label.pack(padx=scale_size(self.root,10), pady=(0, scale_size(self.root,10)), anchor="center")
    ...
    def on_key_press(self, event):
        ...
        # 履带：方向键
            ...
            self.send_command("UF" if getattr(self, "track_high_speed", False) else "TF", "履带前进", target="motion")
        elif key == 'down':
            self.send_command("UB" if getattr(self, "track_high_speed", False) else "TB", "履带后退", target="motion")
        elif key == 'left':
            self.send_command("UL" if getattr(self, "track_high_speed", False) else "TL", "履带左转", target="motion")
        elif key == 'right':
            self.send_command("UR" if getattr(self, "track_high_speed", False) else "TR", "履带右转", target="motion")
        # 其他键不做映射
    ...
    def on_key_release(self, event):
        ...
        # 履带松开方向键停止
            ...
            self.send_command("US" if getattr(self, "track_high_speed", False) else "TS", "履带停止", target="motion")
    ...
    def on_key_release(self, event):
        key = event.keysym.lower()
        # Linux下autorepeat会导致KeyRelease和KeyPress交替出现，这里用after延迟判定
        def real_release():
            if key in self.pressed_keys:
                return  # 说明期间又按下了，不是真正松开
            # 推进器松开 w/a/d 停止
            if key in ('w', 'a', 'd'):
                self.send_command("WS", "推进器停止", target="motion")
            # 履带松开方向键停止
        ...
        if key in ('w', 'a', 'd', 'up', 'down', 'left', 'right'):
            # 20ms后检查是否真的松开
            if key in self._key_release_timers:
                self.root.after_cancel(self._key_release_timers[key])
            self._key_release_timers[key] = self.root.after(20, real_release)
